            ...
            async function loadLessons() {
                try {
                    ...
                    const lessonsHTML = lessonsData.map((lesson, index) => \`
                        <div class="border rounded-lg p-4 hover:border-purple-300 hover:shadow-md transition">
                            <div class="flex items-start">
                                <div class="flex-shrink-0 w-12 h-12 bg-purple-100 rounded-full flex items-center justify-center text-purple-600 font-bold">
                                    \${lesson.lesson_number}
                                </div>
                                <div class="ml-4 flex-1">
                                    <h3 class="text-lg font-semibold text-gray-900 mb-2">
                                        \${lesson.title}
                                        \${lesson.is_free || lesson.is_free_preview ? '<span class="ml-2 px-2 py-1 bg-green-100 text-green-800 text-xs font-semibold rounded">무료</span>' : ''}
                                    </h3>
                                    <p class="text-gray-600 text-sm mb-3">\${lesson.description || '차시 설명이 없습니다.'}</p>
                                ...
                                <div class="ml-4">
                                    \${enrollment || lesson.is_free || lesson.is_free_preview ? \`
                                        <button onclick="playLesson(\${lesson.id})" class="bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 transition">
